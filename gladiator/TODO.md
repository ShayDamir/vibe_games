...
- [x] **Hidden gear sets ("bonds")**: a matching weapon + armor pair forges a hidden *passive* bonus — outclassed gear gets a second life. 5 sets (Bare-Knuckle Brawler fists+leather, Oak Skirmisher club+leather, Iron Vanguard sword+iron, Rune Channeler staff+magic, Dragon Crusher hammer+dragon), stat-only, player-only (foes never bond). Hidden like arcana: the Arcana codex gained a "Bonds" section (❓ until the pair is first equipped together → "BOND FORGED" toast), a gold bond icon on the player panel + hub while the pair is worn, `save.discoveredSets[]` persisted. `SETS`/`activeSetKey()` in `data.js`, `applySet()`/`actionCost` in `combat.js`, `checkSetForge()` in `game.js`; smoke 7b asserts every bonus against a baseline rebuild.
