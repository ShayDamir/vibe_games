...
- After every word a reveal card shows the translation and the gender rule — a rule chip, a red **EXCEPTION** chip, or a red **NO RULE** chip ("memorize it with the article"). Compound words additionally show which word they take the gender of, and words with a `mnemo` show it in gold under the chip. The card stays up longer after a wrong answer or miss (5.5s vs 2.7s) so it can be read and memorized. Space pauses to read.
